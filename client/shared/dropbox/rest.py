...
class RESTClient(object):
    ...
    @staticmethod
    def request(method, url, post_params=None, body=None, headers=None, raw_response=False, callback=None):
            ...
            if not hasattr(body, 'read'):
                ...
                if clen != None:  #clen == 0 is perfectly valid. Must explicitly check for None
                    ...
                    while data:
                        ...
                        percent = (bytes * 100) / filesize
                        if (callback != None):
                            callback(percent)
                        #print("%02d complete" % percent)
                        conn.send(data)
                        data=body.read(BLOCKSIZE)
    ...
    @classmethod
    def PUT(cls, url, body, headers=None, raw_response=False, callback=None):
        """Perform a PUT request using RESTClient.request"""
        assert type(raw_response) == bool
        return cls.request("PUT", url, body=body, headers=headers, raw_response=raw_response, callback=callback)
